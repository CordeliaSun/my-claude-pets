...
function normalizePet(raw, index, configDir) {
  ...
  if (typeof raw.balloonImage === 'string' && raw.balloonImage.trim()) {
    const resolved = path.resolve(configDir, expandHome(raw.balloonImage.trim()))
    pet.balloonImage = fs.existsSync(resolved) ? resolved : null
  }
  return pet
}
...
function loadPets(configPath) {
  const pets = []
  const errors = []
  const base = { pets, errors, searchRoots: null, defaultCommand: 'claude', officialQuota: true }
  let raw
  try {
  ...
  if (Array.isArray(parsed)) {
    ...
    if (parsed.officialQuota === false) base.officialQuota = false
  } else {
